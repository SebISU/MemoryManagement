...
# Znacznik czasowy: 2020-10-26 17:15:10.457546
#

OUTDIR		:= build-dir

CC          := gcc
CC_FLAGS    := -Werror=vla -pedantic -DINSIDE_DANTE -Werror -fdiagnostics-color -Wextra -fmax-errors=5 -Wno-error=unused-parameter -D_ANSI_OUTPUT -Wno-error=implicit-fallthrough -Wall -Wno-parentheses -D_GNU_SOURCE -ggdb3 -xc -Wno-error=parentheses -std=c11 
...
LD          := gcc
LD_FLAGS    := -Wl,-Map=main.map -Wl,-cref -Wl,-wrap,main -ggdb3 
LD_LIBS     := -lm -lpthread 
